...
#include "UserStackerTruckProcQiangSheng.h"
#include "ErrCode.h"
#include "Eeprom.h"
#include "IQmathLib.h"
#include "PropProc.h"
...
const static xPdoParameter  sgPdoPara = 
{
	.TpdoPara = {
		...
		{.u8Flag = 1, .u8Type = 0xFF, .u16Period = 100, .u16CanId = 0x261 },
	},
	.RpdoPara = {
...
const static uint8_t u8SeedArray[16]= "V!BWT%EM6dJ8<nPs";     // 获取程序编译时间
...
//	i16Spd = abs(i16Spd);
	...
	if((sgSwiInput.b1Backward == 1)
		&&(((sgSwiInput.b1FENCE1 == 1) &&(sgSwiInput.b1FENCE2 == 1)) ||((sgSwiInput.b1FENCE1 == 0) &&(sgSwiInput.b1FENCE2 == 0)) ))
	{         
	...
	if ((0 == sgSwiInput.b1SafeLock)&&(0 == sgFlagVal.ZhiLi_MOVE))
	{
		...
		{			
			/*lilu 20230819 For Test*/
			{
				...
				i32SetPara(PARA_BmsSoc, sgValvesInfo.b1Gear1SpdFlag);		/*BMS SOC*/
...
//明天看这一段怎么修改	
	...
	if(i32AdcValue > 90)
	{
		u16MotorVal = (((i32AdcValue - 90)*4096)/(4927-80));
	}
	else
	{
		u16MotorVal = 0;
...
static void vSwiMonitor(void)
{
	...
	if(1 == i32LocalDiGet(LIMT_300MM_SPEED))
	{
		SwiInput.b1HIGHTLIMIT = 1;
	}
	...
	if((1 == sgSwiInput.b1HIGHTLIMIT)&&(1 ==sgSwiInput.b1LiftUp))
	{
		sgFlagVal.Hight_Limit = 1;
		u16EepromWrite(PARA_SaveState, sgSaveState.u16Data, 1);
	}
	else if((1 == sgSwiInput.b1HIGHTLIMIT)&&(1 == sgSwiInput.b1LiftDown))
	{
		sgFlagVal.Hight_Limit = 0;
		u16EepromWrite(PARA_SaveState, sgSaveState.u16Data, 1);
	}
	sgSaveState.b1Above1M8 = sgFlagVal.Hight_Limit ;
	...
	else if  (1 == sgSwiInput.b1Slow_Mode)
	{
		sgValvesInfo.b1Gear1SpdFlag = 0;
		sgValvesInfo.b1Gear2SpdFlag = 0;
	...
	else if((1 == sgFlagVal.Hight_Limit) && (0 == sgSwiInput.b1Slow_Mode))
	{
		sgValvesInfo.b1Gear1SpdFlag = 0;
	...
	else if((sgSwiInput.b1FENCE1 == 0) &&(sgSwiInput.b1FENCE2 == 0)&& (0 == sgSwiInput.b1Slow_Mode))
	{
...
//		}
	...
	if (true == u8GetNetTimerOverFlag(TIMER_HourCount))
	{
		...
		if (u16SecCnt >= 360)			/**/
		{
			...
			__disable_irq();	
			gCanSendPdoInfo.CanSend261Info.u8HourLL = u32HourCount & 0xFF;
			gCanSendPdoInfo.CanSend261Info.u8HourLH = (u32HourCount >> 8) & 0xFF;
			gCanSendPdoInfo.CanSend261Info.u8HourHL = (u32HourCount >> 16) & 0xFF;
			gCanSendPdoInfo.CanSend261Info.u8HourHL = (u32HourCount >> 24) & 0xFF;			
			__enable_irq();
		}

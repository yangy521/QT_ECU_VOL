...
#include "UserStackerTruckProcKeliDQY.h"
#include "ErrCode.h"
#include "Eeprom.h"
#include "IQmathLib.h"
#include "PropProc.h"
...
const static xPdoParameter  sgPdoPara = 
{
	.TpdoPara = {
		...
		{.u8Flag = 0, .u8Type = 0x00, .u16Period = 50},
		{.u8Flag = 0, .u8Type = 0x00, .u16Period = 50},
		{.u8Flag = 1, .u8Type = 0xFF, .u16Period = 50, .u16CanId = 0x370},
...
static void vMstSendProc(xMstSendPara *SendData) 
{
	...
	
	{
		...
		{			
			...
			{
				i32SetPara(PARA_ForwardValveCurrent, sgUserInfo.u8BatteryType);		/*Send Motor Value*/
				i32SetPara(PARA_BackValveCurrent, Rev_Speed);		/*Rev Motor Value*/
				i32SetPara(PARA_PropValveCurrent, i32LocalAiGetValue(MOVE_THROTTLE));	/*Prop Current*/
...
/***** 锂电池   ********/
	{
		...
		/*添加相关操作*/
		gCanSendPdoInfo.CanSend260Info.u8ErrorBMS = CanRev2F0InfoLast.BMS_Errcode;
	}
...
/*********** Motor Speed *************/
	...
	if(i32AdcValue > 10)
	{
		u16MotorVal = (4096*i32AdcValue)/4910;
	}
	else
	{
		u16MotorVal = 0;
...
static void vSwiMonitor(void)
{
	...
	if(0 == i32LocalDiGet(Footbrake_SWI))
	{
  ...
  gCanSendPdoInfo.CanSend260Info.u8Soc = Batter_SOC;
	if(Batter_SOC < 20)      //高度限速，限制为四档速度
	{
...
//	vPropErrReg(vPropErrCallBack);
	...
	sgUserInfo.u8BatteryType = i32GetPara(BATTER_TYPE);
...
typedef union
{
	...
	struct
	{
		uint16_t b1SpeedRate1: 1;
		uint16_t b1SpeedRate2: 1;
		uint16_t b1SpeedRate3: 1;
		uint16_t b12Reserve: 13;
	};
}xSwiInput;
...
/*PDO接收待完善*/
xCanRevPdoInfo gCanRevPdoInfo; 
static xCanRev260Info CanRev260InfoLast;				
/*PDO发送待完善*/
xCanSendPdoInfo gCanSendPdoInfo;
...
static void vMstSendProc(xMstSendPara *SendData) 
{
	...
	/*Move Mode*/
	if((0 != sgSwiInput.u16data)&&(gCanRevPdoInfo.CanRevInfo260.u8Soc > 10))
	{
		SendData->u8TargetHigh = u16MotorVal >> 8;
		SendData->u8TargetLow = u16MotorVal;
		SendData->b1ServoOn = 1;
	...
	
	{
		...
		{			
			/*lilu 20230819 For Test*/
			{
				...
				i32SetPara(PARA_BackValveCurrent, Rev_Speed);		/*Rev Motor Value*/
				i32SetPara(PARA_PropValveCurrent, CanRev260InfoLast.u8Soc);	/*Prop Current*/
//				i32SetPara(PARA_LiftValveCurrent, ((gCanSendPdoInfo.CanSend260Info.u8HourCountH >> 8)|(gCanSendPdoInfo.CanSend260Info.u8HourCountL)));		/*Send Pump Value*/
				i32SetPara(PARA_OnOffValveCurrent, SendData->buf[2]);			/*Send Status*/
				i32SetPara(PARA_LoadRate, sgValvesInfo.u8Data[0]);				/*Valve NoFlag*/
				i32SetPara(PARA_CalibrationStatus, sgValvesInfo.u8Data[1]);		/*Value State*/
...
/***** 转向360 *******/
	{
		memcpy((char*)CanRev260InfoLast.u8Data, (char*)gCanRevPdoInfo.CanRevInfo260.u8Data, sizeof(CanRev260InfoLast));
		/*添加相关操作*/
  }
}
...
/*********** Motor Speed *************/	
	...
	if(0 != sgSwiInput.u16data)
	{
		u16MotorVal = (uint16_t)((4096 * u8SpeedRate)/100);
	}
	else
	{
		u16MotorVal = 0;
...
static void vSwiMonitor(void)
{
	...
	if(1 == i32LocalDiGet(Speed_rate1))
	{
		SwiInput.b1SpeedRate1 = 1;
	}
	
	if(1 == i32LocalDiGet(Speed_rate2))
	{
		SwiInput.b1SpeedRate2 = 1;
	}
	
	if(1 == i32LocalDiGet(Speed_rate3))
	{
		SwiInput.b1SpeedRate3 = 1;
	}
	
	sgSwiInput.u16data = SwiInput.u16data;
	
	if(1 == SwiInput.b1SpeedRate1)      //高度限速，限制为四档速度
	{
	...
	else if(1 == SwiInput.b1SpeedRate2)
	{
		sgValvesInfo.b1Gear1SpdFlag = 0;
	...
	else if(1 == SwiInput.b1SpeedRate3)
	{
		sgValvesInfo.b1Gear1SpdFlag = 0;
		sgValvesInfo.b1Gear2SpdFlag = 0;

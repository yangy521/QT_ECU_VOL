...
//	}
	if(1 ==SwiInput.b1SlowRequest)
	{
		...
		if( 0 == SwiInput.b1SafeLock)
		{
			...
			else if((u16ZhiLicnt < u16zhili_time/USER_ECU_PERIOD)||(u8nolimitflag))//进入直立行走状态
			{
				u16ZhiLicnt ++;
...
//			SwiInput.b1Above1M8 = sgSwiInput.b1Above1M8;			/*lilu 20230801 Above1M8 retain*/
...

			{
				...
				static uint8_t u8PumpRecord = 0;//实际输出逻辑
				...
				if((1 == u8ErroDelayFlag)&&(0 !=(u8PumpOut^u8PumpRecord)))//本次输出与上一次输出不一致
				{
					PumpMotorNoAct = 1;
				...
				{ //11.20
					...
					u8PumpActTmp = (sgValvesInfo.b1LiftUpStat<<5) | (sgValvesInfo.b1LiftDownStat<<4) |			
							(sgValvesInfo.b1LegUpStat << 3 )| (sgValvesInfo.b1LegDownStat <<2)|
							(	sgValvesInfo.b1OutriggerUpStat <<1) | sgValvesInfo.b1OutriggerDownStat;
					if((0 == u8PumpActTmp)&&(0 != u8PumpDelayRec))//有历史动作，现在输入动作为0，
					{
						u8PumpDelayFlag =1;
					}
					if((1 == u8PumpDelayFlag)&&(u16PumpDelayCnt < (1000 / USER_ECU_PERIOD)))//有标志位，开始计时
					{
						u16PumpDelayCnt++;
					}
					else//计时结束，清空标志位
...
//							(sgValvesInfo.b1LegUpStat << 3 )| (sgValvesInfo.b1LegDownStat <<2)|
//							(	sgValvesInfo.b1OutriggerUpStat <<1) | sgValvesInfo.b1OutriggerDownStat))//输出不为0
//				{
//					u8PumpActFlag = 1;
//				}
...
static void vCanRevPdoProc(void)
{
	...
	if ((gCanSendPdoInfo.CanSend260Info.u8Soc <= BAT_LOW_ERR_VAL)
			||(0 != (CanRev190InfoLast.u8BMS_ERR_Require & 0x01)))//行走，起升禁止、只下降、提腿降42
	{
		if(gCanSendPdoInfo.CanSend260Info.u8Soc <= BAT_LOW_ERR_VAL)
		{
		...
		u8OutriggerNoActFlag = 1;
				...
				||(0 !=(CanRev190InfoLast.u8BMS_ERR_Require & 0x0A)))//行走全速，限制起升,提腿升41
	{
	...
	if(0 == sgSwiInput.b1SteerLock)//转向报文处理
	{
...
//	u16MotorVal = u16SteerAngleDecSpd(u16MotorVal, abs(gCanRevPdoInfo.CanRevInfo2.i16SteerAngle), &sgUserInfo.SteerAngleDecSpd);
	...
	if (LIFT_MODE_THROTTLE == sgUserInfo.b1LiftMode)
	{
		...
		if (1 == sgSwiInput.b1LiftUp)
		{
			...
			else if (i32AdcValue >= sgUserInfo.u16LiftUpMidValue)		/*小于起升最大*/
			{
			...
			else if (i32AdcValue >= sgUserInfo.u16LiftUpMin)	/*小于起升一半*/
			{
		...
		else if(1 == sgSwiInput.b1LiftDown)
		{
			...
			else if (i32AdcValue >= (sgUserInfo.u16LiftDownMidValue))		/*超过一半*/
			{

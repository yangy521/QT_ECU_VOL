...
#include "USER_SDSaiYiSi_2QBICHE.h"
#include "ErrCode.h"
#include "Eeprom.h"
#include "IQmathLib.h"
#include "PropProc.h"
...
#if ((USER_TYPE == USER_SDSAIYISI_BICHE_ZUO_MOVE) ||  (USER_TYPE == USER_SDSAIYISI_BICHE_YOU_SMOVE))
const static xPdoParameter  sgPdoPara =
{
	.TpdoPara = {
		{.u8Flag = 0, .u8Type = 0x00, .u16Period = 20},
		{.u8Flag = 0, .u8Type = 0x00, .u16Period = 100},
		{.u8Flag = 0, .u8Type = 0x00, .u16Period = 20},
		{.u8Flag = 0, .u8Type = 0x00, .u16Period = 100},
		{.u8Flag = 1, .u8Type = 0xFF, .u16Period = 8, .u16CanId = 0x183 },

	},
	.RpdoPara = {
	#if(USER_TYPE == USER_SDSAIYISI_BICHE_ZUO_MOVE)    //可能接收部分 
		{.b1Flag = 1, .b11CanRevId = 0x226},
		{.b1Flag = 1, .b11CanRevId = 0x326},
		{.b1Flag = 1, .b11CanRevId = 0x1A7},
		{.b1Flag = 1, .b11CanRevId = 0x2A7},
		{.b1Flag = 1, .b11CanRevId = 0x3A7},
		{.b1Flag = 1, .b11CanRevId = 0x4A7},
	#else
		{.b1Flag = 1, .b11CanRevId = 0x227},
		{.b1Flag = 1, .b11CanRevId = 0x000},
		{.b1Flag = 1, .b11CanRevId = 0x1A6},
		{.b1Flag = 1, .b11CanRevId = 0x2A6},
		{.b1Flag = 1, .b11CanRevId = 0x3A6},
		{.b1Flag = 1, .b11CanRevId = 0x4A6},
	#endif		
	},
};
...
typedef union 
{
	...
	struct
	{
		uint8_t b1HoldState: 1;
		uint8_t b1BrakeState: 1;
		uint8_t b1IniTerState: 1;
		uint8_t b1SvonState: 1;
		uint8_t b1Rental: 1;
		uint8_t b11Reserve: 3;		
	};
		
}xLockState;
xLockState sgLockState;
xSaveStateInfo sgSaveState;
			  
static xCanRev22XInfo CanRev22XInfoLast;
static xCanRev32XInfo CanRev32XInfoLast;
...
static int16_t i16VCUMotorSpd =0;
static uint16_t u16PumpOrPropValue =0;
static uint8_t u8Steerangle;
static uint32_t u32HourCount;
static uint16_t Rev_Speed;
static uint8_t	u8PropVal = 0;
static int16_t  i16motorSpdSend =0;
static INT8U	MototDirCmd =0;			//0停止  1 前进  2 后退
...
{	
	...
	else
	{
		...
		u16Current = (uint16_t)(RevData->u8CurrentLow << 8) | RevData->u8CurrentHigh ;	
...
static void vMstSendProc(xMstSendPara *SendData) 
{
	...
	if(2 == MototDirCmd )
	{  
		sgLockState.b1HoldState=1;		
		SendData->b1BackwardReq = 1;
	}
	else if(1 == MototDirCmd )
	{
		sgLockState.b1HoldState=1;
		SendData->b1ForwardReq = 1;
	}
	//如何手动打开制动器？？？
	if((0 == MototDirCmd) ||(0 == CanRev22XInfoLast.b1MotorInterLock))
	{
	...
	/*Lift Mode*/
	if((0 != CanRev32XInfoLast.b1PumpInterLock) || ( 0 != CanRev32XInfoLast.u8PumpSpeedH))
	{
		SendData->u8PumpTarget = (u16PumpOrPropValue*(CanRev32XInfoLast.u8PumpCurrentLimit)*255 /32767) ;
	...
	{			
		/*lilu 20230819 For Test*/
		{
			...
			i32SetPara(PARA_BmsSoc, u8Steerangle);		/*BMS SOC*/
...
static void vCanRevPdoProc(void)
{
	...
	{
		if (0 != memcmp(CanRev22XInfoLast.u8Data, gCanRevPdoInfo.CanRevInfo22X.u8Data, sizeof(CanRev22XInfoLast)))
		{
			memcpy((char*)CanRev22XInfoLast.u8Data, (char*)gCanRevPdoInfo.CanRevInfo22X.u8Data, sizeof(CanRev22XInfoLast));
			i16VCUMotorSpd = (CanRev22XInfoLast.u8MotorSpeedL | CanRev22XInfoLast.u8MotorSpeedH <<8);			
		}
		if (0 != memcmp(CanRev32XInfoLast.u8Data, gCanRevPdoInfo.CanRevInfo32X.u8Data, sizeof(CanRev32XInfoLast)))
		{
			memcpy((char*)CanRev32XInfoLast.u8Data, (char*)gCanRevPdoInfo.CanRevInfo32X.u8Data, sizeof(CanRev32XInfoLast));
			u16PumpOrPropValue = ((CanRev32XInfoLast.u8PumpSpeedH <<8) | CanRev32XInfoLast.u8PumpSpeedL );
		}
...
/*********** Motor Speed *************/
	...
	if(i16VCUMotorSpd<0)
	{
		i16VCUMotorSpd = -i16VCUMotorSpd;
		MototDirCmd = 2;
	}
	else if(i16VCUMotorSpd >0)
		MototDirCmd = 1;
	else
		i16VCUMotorSpd = 0;
	
	u16MotorVal = (i16VCUMotorSpd *4096 / 32768);      
	...
	//最终输出值
	u16MotorVal = (uint16_t)((u16MotorVal * (CanRev22XInfoLast.u8BoradCurrentLimit))/100);
...
//	static INT8U snail_Count =0 ;

//	if(1 == i32LocalDiGet(Toggle_SWI))
//	{
//		SwiInput.b1ToggleSwitch = 1;
//	}	
//	
//	if(1 == i32LocalDiGet(LiftLimit_SWI))
//	{
//		SwiInput.b1LiftLimitSwitch = 1;
//	}
//	
//	if(1 == i32LocalDiGet(DownLimit_SWI))
//	{
//		SwiInput.b1DownLimitSwitch = 1;
//	}
//	
//	if(1 == i32LocalDiGet(EmsStop_SWI))
//	{
//		//SwiInput.b1StopSwitch = 1;
//	}
//	
//	sgSwiInput.u8Date = SwiInput.u8Date;
...
//	//i32LogWrite(INFO, "******DefaultFlag = 0x%x\r\n*********", u16Tmp);
	...
	else
	{
		...
		//sgFlagVal.Hight_Limit = sgSaveState.b1Above1M8;
...
//				vCanId5ACSend();
	...
	{   
		...
		//1AXID
		gCanSendPdoInfo.CanSend1AXInfo.u8DiState1 = 0;
		
		//2AXID

		
		gCanSendPdoInfo.CanSend2AXInfo.u8BatteryCurrentL = u16Current & 0xFF;
		gCanSendPdoInfo.CanSend2AXInfo.u8BatteryCurrentH = (u16Current >> 8) & 0xFF;
//		gCanSendPdoInfo.CanSend2AXInfo.u8BatteryCurrentL =;
//		gCanSendPdoInfo.CanSend2AXInfo.u8BatteryCurrentH = ;
//		gCanSendPdoInfo.CanSend2AXInfo.u8PumpCurrentL =;
//		gCanSendPdoInfo.CanSend2AXInfo.u8PumpCurrentH = ;
		gCanSendPdoInfo.CanSend2AXInfo.u8BatterySoc = u8BatterySoc;
		gCanSendPdoInfo.CanSend2AXInfo.u8Errorcode = u8ErrCode;
		
		//3AXID
//		gCanSendPdoInfo.CanSend3AXInfo.u8MotorCurrentL = ;
//		gCanSendPdoInfo.CanSend3AXInfo.u8MotorCurrentH = ;
//		gCanSendPdoInfo.CanSend3AXInfo.u8BoardTemperatureL = ;
//		gCanSendPdoInfo.CanSend3AXInfo.u8BoardTemperatureH = ;
		gCanSendPdoInfo.CanSend3AXInfo.u8MotorSpeedL = i16MotorSpd& 0xFF;
		gCanSendPdoInfo.CanSend3AXInfo.u8MotorSpeedH = (i16MotorSpd >>8) &0xFF;
//		gCanSendPdoInfo.CanSend3AXInfo.u8MotorTemperatureL =;
//		gCanSendPdoInfo.CanSend3AXInfo.u8MotorTemperatureH  = ;
		
		//4AXID
//		gCanSendPdoInfo.CanSend4AXInfo.u8ErrorFlag = ;
//		gCanSendPdoInfo.CanSend4AXInfo.u8AngleEcuL = ;
//		gCanSendPdoInfo.CanSend4AXInfo.u8AngleEcuH = ;
//		gCanSendPdoInfo.CanSend4AXInfo.u8VerSion = ;
//		gCanSendPdoInfo.CanSend4AXInfo.u8CurrentThrottleL =;
//		gCanSendPdoInfo.CanSend4AXInfo.u8CurrentThrottleH = ;
//		gCanSendPdoInfo.CanSend4AXInfo.u8DoState =;
		
		
		gCanSendPdoInfo.CanSend4AXInfo.u8LockState = sgLockState.u8Data;
		__enable_irq();
	}

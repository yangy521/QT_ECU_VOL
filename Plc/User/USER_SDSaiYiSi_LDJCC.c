...
#include "UserStackerTruckProcSHXIANGONG_AGV.h"
#include "ErrCode.h"
#include "Eeprom.h"
#include "IQmathLib.h"
#include "PropProc.h"
...
#if( (USER_TYPE == USER_SDSAIYISI_LDJCC_MOVE)  ||(USER_TYPE == USER_SDSAIYISI_LDJCC_SMOVE))
const static xPdoParameter  sgPdoPara =
{
	.TpdoPara = {
		...
		{.u8Flag = 1, .u8Type = 0xFF, .u16Period = 8, .u16CanId = 0x283 },
		{.u8Flag = 0, .u8Type = 0xFF, .u16Period = 8, .u16CanId = 0x204 },  //reserve
	},
	.RpdoPara = {
		{.b1Flag = 1, .b11CanRevId = 0x203},
		{.b1Flag = 1, .b11CanRevId = 0x303},
		{.b1Flag = 1, .b11CanRevId = 0x1A3},
...
typedef union
{
	...
	struct
	{
		uint8_t b1ToggleSwitch: 1;
		uint8_t b1LiftLimitSwitch: 1;
		uint8_t b1DownLimitSwitch: 1;
		uint8_t b1StopSwitch: 1;
		uint8_t b4Resever: 4;
	};           
}xSwiInput;
...
static xCanRev203Info CanRev203InfoLast;
static xCanRev303Info CanRev303InfoLast;
static xCanRev1A3Info CanRev1A3InfoLast;
/*PDO接收和发送*/
...
static uint16_t u16VCUMotorSpd =0;
static uint16_t u16PumpOrPropValue =0;
static uint16_t u16PumpOrPropValue2 =0;
...
static void vMstSendProc(xMstSendPara *SendData) 
{
	...
	i32PropValue = _IQ((sgUserInfo.fPropMinCurrent + (sgUserInfo.fPropMaxCurrent - sgUserInfo.fPropMinCurrent) * (u16PumpOrPropValue2*255/1000) / PUMP_RANGE) / PROPD_STD_CURRENT);
																												 
	if (1 == gCanRevPdoInfo.CanRevInfo203.b1Saftsign)		/*add Main Connector*/
	{
	...
	/*Move Mode*/
	//自动模式
	if(0 == sgSwiInput.b1ToggleSwitch)
	{
		if(0 != CanRev203InfoLast.b1BackwardSign )
		{         
			SendData->b1ServoOn = 1;
			SendData->b1BackwardReq = 1;
		}
		else if(0 != CanRev203InfoLast.b1ForwardSign )
		{
			SendData->b1ServoOn = 1;
			SendData->b1ForwardReq = 1;
		}
		
		if((1 == CanRev203InfoLast.b1BrakewardSign) ||(0 == CanRev203InfoLast.b1Saftsign) )
		{
	...
	{
		if(0x01	 == CanRev1A3InfoLast.u8RemoteDir)	
		{         
			SendData->b1ServoOn = 1;
			SendData->b1BackwardReq = 1;
		}
		else if(0xFF == CanRev1A3InfoLast.u8RemoteDir)	
		{
			SendData->b1ServoOn = 1;
			SendData->b1ForwardReq = 1;
		}
		
		if(1 == CanRev1A3InfoLast.b1RemoteEMS)
		{
			SendData->b1ServoOn = 0;
			SendData->b1ForwardReq = 0;
			SendData->b1BackwardReq = 0;
	...
	if((0 != CanRev203InfoLast.b1BackwardSign)||(0 != CanRev203InfoLast.b1ForwardSign) ||(0x01	 == CanRev1A3InfoLast.u8RemoteDir) ||(0xFF == CanRev1A3InfoLast.u8RemoteDir))
	{
		SendData->u8TargetHigh = u16MotorVal >> 8;
		SendData->u8TargetLow = u16MotorVal;
	}
	
	if(1 == sgSwiInput.b1StopSwitch) //开关急停
		sgValvesInfo.u8NoAct |=  (1 << NoAct_EMSSWI);
	else
		sgValvesInfo.u8NoAct &= ~(1 << NoAct_EMSSWI);

	
	/*Lift Mode*/
	if((1 == CanRev303InfoLast.b1LiftUpword) || ( 0x01 == CanRev1A3InfoLast.u8LiftDir))
	{
		SendData->u8PumpTarget = (u16PumpOrPropValue*255 /1000) ;//(u16PumpOrPropValue/1000);
		SendData->b1LiftReq = 1;
		SendData->b1ServoOn = 1;
		vPropSetTarget(PropDriverCh0, 0);
	}
	else if((1 == CanRev303InfoLast.b1LiftDownword) || (0xFF == CanRev1A3InfoLast.u8LiftDir)) 
	{
		SendData->u8PumpTarget = 0;
		SendData->b1LiftReq = 0;
		SendData->b1ServoOn = 1;
		SendData->b1DownReq = 1; 
...
static void vCanRevPdoProc(void)
{
	...
	{
		if (0 != memcmp(CanRev203InfoLast.u8Data, gCanRevPdoInfo.CanRevInfo203.u8Data, sizeof(CanRev203InfoLast)))
		{
			memcpy((char*)CanRev203InfoLast.u8Data, (char*)gCanRevPdoInfo.CanRevInfo203.u8Data, sizeof(CanRev203InfoLast));
			u16VCUMotorSpd =  ((CanRev203InfoLast.u8VCUSpeedH << 8) | (CanRev203InfoLast.u8VCUSpeedL));
		}
		if (0 != memcmp(CanRev303InfoLast.u8Data, gCanRevPdoInfo.CanRevInfo303.u8Data, sizeof(CanRev303InfoLast)))
		{
			memcpy((char*)CanRev303InfoLast.u8Data, (char*)gCanRevPdoInfo.CanRevInfo303.u8Data, sizeof(CanRev303InfoLast));
			u16PumpOrPropValue = ((CanRev303InfoLast.u8LiftSpeedH << 8) | (CanRev303InfoLast.u8LiftSpeedL));
			u16PumpOrPropValue2 = ((CanRev303InfoLast.u8DownSpeedH <<8 ) | (CanRev303InfoLast.u8DownSpeedL));
			u8Steerangle = CanRev303InfoLast.u8SteerSpeedrate;    //转向时最大速度设定 暂未处理
		}
		if (0 != memcmp(CanRev1A3InfoLast.u8Data, gCanRevPdoInfo.CanRevInfo1A3.u8Data, sizeof(CanRev1A3InfoLast)))
		{
			memcpy((char*)CanRev1A3InfoLast.u8Data, (char*)gCanRevPdoInfo.CanRevInfo1A3.u8Data, sizeof(CanRev1A3InfoLast));
		}			
	}                                    
}
...
/*********** Motor Speed *************/
	...
	//手动和自动
	if(0 == sgSwiInput.b1ToggleSwitch)
		u16MotorVal = (u16VCUMotorSpd *4096 / 2534);      
	else
		u16MotorVal = (2400 *4096) /2534; 		//手动模式	
	//u16MotorVal = u16SteerAngleDecSpd(u16MotorVal, abs(i16SteerAngle), &sgUserInfo.SteerAngleDecSpd);
	

	if(1 == sgSwiInput.b1ToggleSwitch)
	{
		if(0x01	 == CanRev1A3InfoLast.u8RemoteDir)
		{			
...
//		{
//			if (1 == CanRev203InfoLast.b1Saftsign)
//			{
//				u16SecCnt++;
//			}
//		}
...
//	}
	...
	{   
		__disable_irq();
//		if(0 != CanRev203InfoLast.b1ForwardSign)
//			i16motorSpdSend = -i16MotorSpd;
//		else if(0 != CanRev203InfoLast.b1BackwardSign)
			i16motorSpdSend = i16MotorSpd;
		//183ID
		gCanSendPdoInfo.CanSend183Info.u8MoveSpeedL = i16motorSpdSend & 0xFF;
		gCanSendPdoInfo.CanSend183Info.u8MoveSpeedH =  (i16motorSpdSend >>8) & 0xFF;
		gCanSendPdoInfo.CanSend183Info.u8ErrorCode =0;
		gCanSendPdoInfo.CanSend183Info.b1HandSwiState = 1;
		gCanSendPdoInfo.CanSend183Info.b1MainDriver =1;
		//283ID
		gCanSendPdoInfo.CanSend283Info.u8MotorTempL = 0x16;
		gCanSendPdoInfo.CanSend283Info.u8MotorTempH = 0x00;
		gCanSendPdoInfo.CanSend283Info.u8ControlCurrentL = 0x16;
		gCanSendPdoInfo.CanSend283Info.u8ControlCurrentH = 0x00;
		gCanSendPdoInfo.CanSend283Info.u8BatterSOC =0x64;
		__enable_irq();
	}

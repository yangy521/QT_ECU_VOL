...
#include "UserStackerTruckProc_MaXiEr.h"
#include "ErrCode.h"
#include "Eeprom.h"
#include "IQmathLib.h"
#include "PropProc.h"
...
static void vMstSendProc(xMstSendPara *SendData) 
{
	...
	if(0 != u16MotorVal)
	{
		...
		{
			if (1 == sgSwiInput.b1Forward)
			{
				SendData->b1ServoOn = 1;
				SendData->b1BackwardReq = 1;
			}
			else if (1 == sgSwiInput.b1Backward)
			{
				SendData->b1ServoOn = 1;
		...
		else
		{
			...
			else if(1 == sgSwiInput.b1Forward)
			{
				SendData->b1ServoOn = 1;
				SendData->b1ForwardReq = 1;
			}
			...
			if((1 == sgSwiInput.b1Backward)||(1 == sgSwiInput.b1Forward))
			{
	...
	if ((1 == sgSwiInput.b1Ems) && ((1 == SendData->b1ForwardReq) || (1 == SendData->b1EmsReq)))  
	{
		SendData->b1EmsReq = 1;
		SendData->b1ForwardReq = 1;
	}
	...
	if ((i32PropValue != 0) && (1 == sgSwiInput.b1LiftUp) && (u8GetBatterySoc() > 5))
	{
		SendData->b1ServoOn = 1;
		SendData->b1LiftReq = 1;
		vPropSetTarget(LIFTDOWN_VALVE, DRIVER_CLOSE);
	}
	else if(1 == sgSwiInput.b1LiftDown)
	{	
		vPropSetTarget(LIFTDOWN_VALVE, i32PropValue);
	}
	else if ((0 == sgSwiInput.b1LiftUp) && (0 == sgSwiInput.b1LiftDown))
	{
	...
	{
		SendData->u8PumpTarget = i32PropValue;
	}
	...
	if (1 == sgSwiInput.b1Ems)
	{
		i32DoPwmSet(HORN, DRIVER_OPEN);
	}
	else 
		i32DoPwmSet(HORN, DRIVER_CLOSE);
	
	
				
	if (1 == i32LocalDiGet(HORN_SWI))
	{
		i32DoPwmSet(HORN_PUMP, DRIVER_OPEN);
	}
	else 
	{
		i32DoPwmSet(HORN_PUMP,DRIVER_CLOSE);
	}
...
//	}
		...
		{			
			...
			{
				i32SetPara(PARA_ForwardValveCurrent, i32PropValue);//SendData->u8PumpTarget);		/*Send Motor Value*/
				...
				i32SetPara(PARA_LiftValveCurrent, (SendData->u8TargetLow));		/*Send Pump Value*/
				i32SetPara(PARA_PropValveCurrent, SendData->u8TargetHigh);			/*Send Status*/
				...
				i32SetPara(PARA_TurnRightValveCurrent, (uint16_t)i32LocalAiGetValue(AI_B_AI2_R));	
				...
				i32SetPara(PARA_BmsSoc, u8GetBatterySoc());		/*BMS SOC*/
				i32SetPara(PARA_BrakeValveCurrent, u8GetBatterySoc());
				i32SetPara(PARA_LiftValveCurrent, u16MotorVal);
...
/*********** Motor Speed *************/
	...
	if((1 == sgSwiInput.b1Backward)||(1 == sgSwiInput.b1Forward) && (1 == sgSwiInput.b1SafeLock))
	...
	/*电量限制*/
	if (u8GetBatterySoc() < BAT_LOW_WARING_VAL + 1)
	{
		i32ErrCodeSet(BAT_LOW_2_ERR);
	}
	else if (u8GetBatterySoc() > BAT_LOW_WARING_VAL)
	{
	...
	if ((1 == sgValvesInfo.b1Gear3SpdFlag) || (u8GetBatterySoc() < BAT_LOW_ERR_VAL + 1))
	{
	...
	if (((1 == sgSwiInput.b1LiftUp) || (1 == sgSwiInput.b1LiftDown))  && (1 == sgSwiInput.b1SafeLock))
	{
		i32AdcValue = i32LocalAiGetValue(LIFT_THROTTLE);
		i32PropValue = i32AdcValue *255/4950;
	}
	else 
	{
		i32PropValue = 0;
		...
		/*lilu 20230823 油泵电机之间的相互动作做到互斥，同时动作时，复位相关动作*/
			{
				...
				if ((0 == sgSwiInput.b1LiftUp) && (0 == sgSwiInput.b1LiftDown) && \
					(0 == u8PumpOrPropValue))
				{
...
{	
	...
	/*lilu 20230823 add user mode*/
	{
		...
		sgUserInfo.b1ForwardBackwardReverse = (u16Tmp >> 5) & 0x01;

#ifndef _USER_CAN_KELIZJDQY_MOVE_CAN_ECU_
#define _USER_CAN_KELIZJDQY_MOVE_CAN_ECU_
#include "stdint.h"
...
typedef union
{
	...
	struct
	{
		uint8_t BMS_VolL;
		uint8_t BMS_VolH;
		uint8_t BMS_CurrentL;
		uint8_t BMS_CurrentH;
		uint8_t BMS_SOC;
		uint8_t BMS_Cap;
		uint8_t BMS_Errcode;
		uint8_t u8Reserve;
...
typedef struct
{
	...
	union
	{
		...
		/*此处添加要发送的结构体类型， for example： xCanSend23CInfo CanSend23CInfo*/
	xCanSend370Info CanSend370Info;
	};
